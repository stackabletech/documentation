...
You can either host your own repository, if you so choose, or specify our public repository for convenience.
...
=== Install Operators
Depending on which products you want to manage with the Stackable platform you will want to install at least one of our operators.
...
If you have provided a KUBECONFIG for the root user in `/root/.kube/config` then the agent will use this, if you decided against a _global_ KUBECONFIG, you will need to specify where the config should be read from.
...
When you run `kubectl nodes get` on the orchestrator, you should see the objects that all your agents added to indicate their readiness.
