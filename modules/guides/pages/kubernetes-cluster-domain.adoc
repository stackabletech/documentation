...
If the environment variable `KUBERNETES_CLUSTER_DOMAIN` (or the helm property `kubernetesClusterDomain`) are not set / overridden, the operator will default the cluster domain to `cluster.local`.
