...
<.> Indent 2 spaces (not 4, also for Python/Java/Rust etc.) to save horizontal space.
...
<.> Prefix the command line with the dollar sign (`$`) which will copy the line in the 'Copy' functionality in the UI.
<.> Do _not_ prefix output lines, so they are not copied.
