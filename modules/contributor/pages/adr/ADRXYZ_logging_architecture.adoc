...
**hiearchical configuration** - The actual code that is running the logging might is running inside a container for a specific role group for a role. Also, software oft supports logging configuration for specific modules:
...
Different configurations at role level or for different role groups:
...
**console vs. file** - We want to have different log levels (and possibly other settings) for console (stdout) and file (aggregator) output. This makes debugging easier, without also filling up the log aggregator with very chatty logs.
