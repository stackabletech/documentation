...
spec:
  provider:
    oidc:
      ...
      # => We only start with Keycloak and Custom in v1, but might extend it in the future
      provider: Keycloak / Google / Github / Azure / Openshift / Okta / Custom # mandatory enum, defaults to Custom
      tls:
        verification:
          none: {}
---
...
### CHOSEN
        ...
        # OR
        oidcClientCredentialsSecret: trino-keycloak-client # mandatory (% anonymous client connection)
        oidcExtraPath: /.well/know/foo # mandatory

        # OR
      - oidc:
          authenticationClass: keycloak # snafu error when types do not match
          clientCredentialsSecret: trino-keycloak-client
          extraPath: /.well/know/foo

        # OR (discarded)
        secret:
          oidc: trino-keycloak-client
        oidcExtraPath: /.well/know/foo

        # OR (discarded)
        secret: trino-keycloak-client
        oidcExtraPath: /.well/know/foo
        # Might cause problems when users switch from LDAP to OIDC
        /stackable/secret/trino-keycloak-client/bindUser
        # vs
        /stackable/secret/trino-keycloak-client/clientId
