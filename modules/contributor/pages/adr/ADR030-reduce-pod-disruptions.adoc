...
Kubernetes has a multiple concepts to try to reduce this to a minimum.

== Considered Options

1. Use PodDisruptionBudget

=== 1. Use PodDisruptionBudget

1. We must deploy a https://kubernetes.io/docs/tasks/run-application/configure-pdb/[PodDisruptionBudgets] alongside all the StatefulSets (and Deployments in the future) to restrict pod disruptions.
