...
Additionally users need to be able to configure their own affinity rules on a
...
spec:
  ...
  config:
    ...
    nodeAffinity: # Whole struct is atomic. When you set something below this you are one your own
      nodeAntiAffinity: null # We don't set any nodeAntiAffinity as a default, but can be set from the user
      nodeAffinity: null
      nodeSelector: null
  servers:
    roleGroups:
