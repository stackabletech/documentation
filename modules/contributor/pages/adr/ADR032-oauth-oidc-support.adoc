...
Does not support reading an OIDC discovery URL but needs: API base URL, auth URL, token URL.
Also the Superset client wants the _email_, _profile_ and _oidc_ scope.
We _could_ also implement fab-oidc support for Superset, so it has proper OIDC support.
This however needs maintenance work from us.
...
spec:
  provider:
    oidc:
      ...
      # Optional port number to use. If unspecified, this will default to 80 if TLS is disabled, 443 otherwise.
      port: $KEYCLOAK_PORT

      # Optional root path appended to the hostname. This defaults to "/".
      rootPath: /realms/master

      # User can specify whatever scopes they want, these three are recommended for OIDC.
      scopes: [ openid, email, profile ]

      # Optional provider hint. If unspecified, the product will not enable any known quirks and will assume OIDC works as it is intended to work.
  ...
  clusterConfig:
    ...
    authentication:
      - authenticationClass: keycloak / open-ldap
        oidc:
          # A reference to the OIDC client credentials secret, which consists of a client id and secret
          clientCredentialsSecret: trino-keycloak-client

          # Additional scopes required for this specific product. It will get merged with the above configured scopes
          extraScopes: [ groups ]
---
apiVersion: v1
kind: Secret
  ...
  clusterConfig:
    ...
    authentication:
      - authenticationClass: open-ldap
        ldap:
          # Optional. Only required for some products. In the future this will be replaced by the key "bindUserSecret".
