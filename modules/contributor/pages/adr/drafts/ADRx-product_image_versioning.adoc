...
* Option 4 was chosen
** Sönke didn't want to marry operator and image version too much
* Remaining question: how far do we want to automate this
** Start with: not at all
** Investigate how far we can do this later
*** Load compatibility from configmap
*** List Tags and select best matching one
* Docker-images repo remains as is
** Git tags will look like this:
*** kafka2.8.0_stackable0.1.0
*** kafka2.9.0_stackable0.1.0
*** kafka2.9.0_stackable0.1.1
** Docker registry tags will look like this:
*** 2.8.0_stackable0.1.0
*** 2.9.0_stackable0.1.0
*** 2.9.0_stackable0.1.1
* Initially these versions will be referred to directly from the CRDs, which means the user will have to pick a working stackable image version from the compatibility matrix
* Image versions are only comparable when combined with the product version _0.1.0_ in kafka2.8.0_stackable0.1.0 and kafka2.9.0_stackable0.1.0 are two completely different versions _0.1.0_
* CI will be changed:
...
* FROM images will be pinned to a hash, so we have a defined start point
** since we run yum update and pip upgrade etc. images are not 100% reproducible anyway
** There shouldn't be a need to rebuild a tag anyway though




Special Case: Different upstream Dockerfiles for a product
