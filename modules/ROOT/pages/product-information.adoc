...
The product also includes the xref:opa:index.adoc[Open Policy Agent], xref:commons-operator:index.adoc[Commons], xref:secret-operator:index.adoc[Secret] and xref:listener-operator:index.adoc[Listener] Operators.
...
and the following versions are supported:
