...
The logging framework (added to the platform in Release 23.1) offers a consistent custom resource configuration and a separate, persisted sink (the default implementation is Opensearch but is interchangeabe). This has now been rolled out across all operators. See this https://docs.stackable.tech/home/stable/contributor/adr/adr025-logging_architecture[ADR] and this https://docs.stackable.tech/home/stable/concepts/logging.html[concepts page] for more information.

Service Type::

The Service type can now be specified in all products. This currently differences between the internal ClusterIP and the external NodePort and is forward compatible with the [ListenerClass](https://docs.stackable.tech/home/stable/listener-operator/listenerclass.html) for the automatic exposure of Services via the Listener Operator. This change is breaking. For security reasons the default is set to the cluster-internal (ClusterIP) ListenerClass. A cluster can be made accessible from outside of Kubernetes by setting clusterConfig.listenerClass to external-unstable (NodePort) or external-stable (LoadBalancer).
