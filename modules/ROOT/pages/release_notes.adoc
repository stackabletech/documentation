...
It is now possible to declare explicit resources on all containers.
...
airflow: [BREAKING] Consolidated remaining top-level config options to clusterConfig (#271).
superset:       [BREAKING] Moved all top level config options to clusterConfig. Authentication is now provided via an array of AuthenticationClasses and additional properties (#379).
trino: [BREAKING]: Reworked authentication mechanism: TheclusterConfig.authentication now requires a list of AuthenticationClass references instead of the MultiUser and LDAP separation (#434).
