...
The focus in this platform release is on the support of default/custom affinities and the status field, as well as the rollout of log aggregation across the remaining operators. Additionally, all operators have been adapted and tested for compatibility with Openshift clusters (versions 4.10 and 4.11). Several operators from the 23.1 platform release were already certified against Openshift: over the coming weeks this certification process will now be extended across all operators and will be specific to each platform-release.
...
Operators of the Stackable Data Platform create, manage and delete Kubernetes resources: in order to easily query the health state of the products - and react accordingly - Stackable Operators use the several predefined condition types to capture different aspects of a product's availability. See this https://docs.stackable.tech/home/stable/contributor/adr/ADR027-status[ADR] for more information.

Default / Custom Affinities::

In Kubernetes there are different ways to influence how Pods are assigned to Nodes. In some cases it makes sense to co-locate certain services that talk a lot to each other, such as HBase regionservers with HDFS datanodes. In other cases it makes sense to distribute the Pods among as much Nodes as possible. There can also be some additional requirements, such as placing important services - such as HDFs namenode - in different racks, datacenter rooms or even datacenters. This release supports default affinities that should suffice for many scenarios out-of-the box as well as enabling the configuration of custom affinity rules at a role and/or role-group level. See this https://docs.stackable.tech/home/stable/contributor/adr/ADR026-affinities[ADR] for more information.

Log Aggregation::

The logging framework (added to the platform in Release 23.1) that offers a consistent custom resource configuration and a separate, persisted sink, has been rolled out across all operators. See this https://docs.stackable.tech/home/stable/contributor/adr/ADR023-product-image-selection.html[ADR] and this https://docs.stackable.tech/home/stable/concepts/product_image_selection.html[concepts page] for more information.

Service Type::

The Service type can now be specified: this enables a later switch to using ListenerClasses for the exposure of Services. This change is breaking, because - for security reasons - the default is set to the cluster-internal ListenerClass. A cluster can be made accessible from outside of Kubernetes by setting clusterConfig.listenerClass to external-unstable or external-stable.
...
No product versions have been deprecated in platform release.
...
This is a condensed form of the https://docs.stackable.tech/stackablectl/stable/demos/data-lakehouse-iceberg-trino-spark.html[data-lakehouse-iceberg-trino-spark] demo focusing on using the lakehouse to store and modify data. It focuses on the Trino and Iceberg integration and should run on a local workstation.
...
As mentioned above, specifying the service type is a breaking for all operators.
The default value is set to the `cluster-internal` `ListenerClass`: if you need your cluster to be accessible from outside of Kubernetes then set `clusterConfig.listenerClass`
...
```
This is an example for Trino, but it is the same pattern across all operators.
...
* Dropped support for old spec.{driver,executor}.nodeSelector field. Use spec.{driver,executor}.affinity.nodeSelector instead - part of https://github.com/stackabletech/issues/issues/323[Deploy default and support custom affinities in our operators]
...
The reason for this is that helm will uninstall the operators but not the CRDs. This best done by deleting the CRDS rather than using kubectl apply, as this will cause some annotations to be concatenated and eventually hit a field size limit.
...
You can use the following command to uninstall all operators that are part of the release 23.1:
...
To install the release 23.4 run
