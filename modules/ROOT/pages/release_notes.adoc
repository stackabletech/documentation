...
It is now possible to add custom settings which specify elements of a pod template (service, stateteful set etc.) on roles or rolegroups, which the operator then merges with the objects it writes before actually applying them. Have a look at https://docs.stackable.tech/home/stable/concepts/overrides.html#_pod_overrides[the documentation] for more details.
