...
* https://github.com/stackabletech/nifi-operator/pull/303[Apache NifFi]
...
Afterwards you will need to remove all the CustomResourceDefinitions installed by the Stackable Platform.
Please note that this will remove all your installed CustomResources, so make sure to save them.

[source,console]
----
$ kubectl get crd -o name | grep '\.stackable\.tech$' | xargs kubectl delete
----
