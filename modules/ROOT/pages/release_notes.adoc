...
This is the first release of the platform, bringing operators for 12 products as well as stackablectl, the commandline tool to interact with stackable components. All operators spin up production ready product instances. Also there are some common features across all operators, such as monitoring, service discovery and configuration overrides. Find the <<Platform features>>, <<_stackablectl,stackablectl features>> and <<_operators>> below.
