...
[#openshift-notes]
== Notes on OpenShift
SDP operators are certified for the OpenShift platform ans can be installed from the OperatorHub.

NOTE: Install the SDP operators in a namespace/project called `stackable-operators`. Installing the operators anywhere else may lead to unwanted efects of even faulty installations. This restriction is caused by limitations in the Operator Bundle Format (OPM) used by OpenShift.
...
In addition, You can restrict the operator to watch a specific namespace. By default, the operator watch all namespaces. For example, to restrict the Kafka operator to watching only a namespace called `kafka-namespace` you add the following properties to the Subscription manifest:
