...
[source,bash]
----
kubectl exec -i -t simple-zk-server-primary-0 -- bin/zkCli.sh -server localhost:2181
----
...
=== Apache Kafka
To test Kafka we'll create a topic, and verify that it was created. By default, Stackable enabled TLS on Kafka so we will need to configure the Kafka client with the correct connection properties.

[source,bash]
----
kubectl exec -i -t simple-kafka-broker-brokers-0 -c kafka -- \
  bash -c "echo -e \"security.protocol=SSL\nssl.truststore.location=/stackable/tls_internal/truststore.p12\nssl.truststore.type=PKCS12\nssl.truststore.password=changeit\" > /stackable/config.properties"
----
...
[source,bash]
----
kubectl exec -i -t simple-kafka-broker-brokers-0 -c kafka -- bin/kafka-topics.sh \
  --bootstrap-server simple-kafka-broker-brokers.default.svc.cluster.local:9092 \
  --create --topic demo --command-config /stackable/config.properties
----
...
----
kubectl exec -i -t simple-kafka-broker-brokers-0 -c kafka -- bin/kafka-topics.sh \
  --bootstrap-server simple-kafka-broker-brokers.default.svc.cluster.local:9093 \
  --list --command-config /stackable/config.properties
----
