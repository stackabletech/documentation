...
* If you really want to get up and running there is a quickstart script that will install services on a single node available in GitHub - https://github.com/stackabletech/stackable-utils.
...
Since this is a walkthrough on how to install Stackable and we’re not building a production cluster, we can be fairly frugal with our resources. We’ll be using Ubuntu 20.04 running in VirtualBox in this walkthrough and we recommend that you do the same as this will help you follow the instructions in this guide. It is assumed that you will know how to perform systems administration tasks such as configuring virtual machines, setting the hostnames and configuring name resolution. The virtual machines will require internet access to download the software.
...
=== Apache Spark
Apache Spark is a little more complex since we're deploying multiple different service types. We will deploy one Spark master, worker and history server on individual nodes . Note that we have used the hostname to match the individual node onto which each service is deployed. If your hostnames differ from the ones given earlier in this guide then you should update the service descriptor accordingly.
      ...
      masters:
        roleGroups:
          default:
            selector:
              matchLabels:
                kubernetes.io/hostname: node1.stackable
            replicas: 1
            config:
              masterPort: 7078
      ...
      workers:
        roleGroups:
          2core2g:
            selector:
              matchLabels:
                kubernetes.io/hostname: node2.stackable
            replicas: 1
            config:
              cores: 2
              memory: "2g"
      ...
      historyServers:
        roleGroups:
          default:
            selector:
              matchLabels:
                kubernetes.io/hostname: node3.stackable
            replicas: 1
            config:
...
You can check the status of the services using `kubectl get pods`. This will retrieve the status of all pods running in the default namespace.
    ...
    spark-simple-default-master-node1       0/1     Registered   0          2m40s
