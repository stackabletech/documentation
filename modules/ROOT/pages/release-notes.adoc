...
* Apache HDFS: built from source from release 24.3
...
This release is the first fully multi-platform release of the Stackable Data Platform. Each image has a manifest list which wraps the architecture-specific image. The status is still experimental, as we work to fine-tune the necessary workflows.

Security::

Support added for OIDC with/without TLS has been added to Apache Druid in this release.

NOTE: SDP now provides OIDC-support for Druid, Superset and Trino

In this release we provide experimental HBase 2.6.0 support with a new experimental policy based authorizer (with OPA). Check here for xref:hbase:usage-guide/security.adoc#_authorization[HBase authorization with OPA documentation] but do not rely on its API as it might change in breaking ways during the experimental phase.
...
* Apache Spark: documentation has been showing how to provision Spark dependencies
...
* Apache HBase: support for exporting snapshots to S3. The HBase image now depends on the Hadoop image and the required AWS JARs are copied from there to the HBase image. The script https://github.com/stackabletech/docker-images/blob/main/hbase/stackable/bin/export-snapshot-to-s3.env[export-snapshot-to-s3] makes exporting easier, and this https://github.com/stackabletech/hbase-operator/tree/main/tests/templates/kuttl/snapshot-export[integration test] shows to use it.

* Apache HBase: support for Omid versions 1.1.1 and 1.1.2
...
* Open Policy Agent: support enabling decision logs
...
* Apache HBase: remove sed calls to replace KRB REALM in configuration. IMPORTANT: This means the operator will ONLY work with HBase versions that are compiled against Hadoop 3.x as of now
...
* Apache Nifi: use config-utils for text-replacement of variables in configs. This fixes escaping problems, especially special characters are included in the password
* Secret operator: the TLS CA Secret is now installed into the Namespace of the operator (typically `stackable-operators`), rather than `default`
...
* https://github.com/stackabletech/hdfs-operator/pull/545[Apache Hadoop: 3.4.0]
...
Of the changes mentioned above, the following are breaking (or could lead to breaking behaviour):
...
* Apache Kafka: remove field/arg controller_config from kafka_controller::Ctx struct and create_controller function
* Secret operator: the TLS CA Secret is now installed into the Namespace of the operator (typically `stackable-operators`), rather than `default`
* Apache Spark: CPU resources are now applied correctly (instead of being rounding to the next whole number). This might affect existing jobs, as they may have e.g. only 200m CPU resources requested instead of the 1000m it had thus far, meaning they might slow down significantly (i.e. possible breaking *behaviour*)
* Trino: change the username which triggers graceful shutdown from `admin` to `graceful-shutdown-user` for greater clarity (e.g. in the Trino policies). This is a breaking change because users need to ensure that `graceful-shutdown-user` has the required permissions to initiate a graceful shutdown. The privileges required for graceful shutdowns are granted to the admin user in the OPA rego rules
...
[INFO ] Uninstalling release 24.3
...
kubectl replace -f https://raw.githubusercontent.com/stackabletech/hello-world-operator/24.7.0/deploy/helm/hello-world-operator/crds/crds.yaml
...
[INFO ] Installing release 24.7
[INFO ] Installing airflow operator in version 24.7.0
[INFO ] Installing commons operator in version 24.7.0
[INFO ] Installing druid operator in version 24.7.0
[INFO ] Installing hbase operator in version 24.7.0
[INFO ] Installing hdfs operator in version 24.7.0
[INFO ] Installing hive operator in version 24.7.0
[INFO ] Installing kafka operator in version 24.7.0
[INFO ] Installing listener operator in version 24.7.0
[INFO ] Installing hello-world operator in version 24.7.0
[INFO ] Installing nifi operator in version 24.7.0
[INFO ] Installing opa operator in version 24.7.0
[INFO ] Installing secret operator in version 24.7.0
[INFO ] Installing spark-k8s operator in version 24.7.0
[INFO ] Installing superset operator in version 24.7.0
[INFO ] Installing trino operator in version 24.7.0
[INFO ] Installing zookeeper operator in version 24.7.0
----
