...
Apache Airflow requires a database to store metaddata. The credetials for this database are configured using the `clusterConfig.credentialsSecret` property. This is `airflow-credentials` in the example below.
...
To assign roles and role groups to nodes, use the `config.affinity` property as described in the https://docs.stackable.tech/home/nightly/contributor/adr/adr026-affinities#_introduce_one_dedicated_attribute[documentation].
