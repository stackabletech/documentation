...
This release is the first fully multi-platform release of the Stackable Data Platform.
Each image has a manifest list which wraps the architecture-specific image.
The status is still experimental, as we work to fine-tune the necessary workflows.
...
* Apache HBase: remove sed calls to replace KRB REALM in configuration. IMPORTANT: This means the operator will ONLY work with HBase versions that are compiled against Hadoop 3.x as of now. This is only relevant if you are building your own images.
