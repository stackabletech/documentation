...
This release is the first official, fully multi-platform release of the Stackable Data Platform. Each image has a manifest list which wraps the architecture-specific image.
...
https://github.com/stackabletech/druid-operator/pull/557

[BREAKING??] Remove sed calls to replace KRB REALM in configuration. IMPORTANT: This means the operator will ONLY work with HBase versions that are compiled against Hadoop 3.x as of now (#531)
https://github.com/stackabletech/hbase-operator/pull/531
...
https://github.com/stackabletech/hive-operator/pull/452

BREAKING: Remove field/arg controller_config from kafka_controller::Ctx struct and create_controller function (#726).
https://github.com/stackabletech/kafka-operator/pull/726
...
https://github.com/stackabletech/nifi-operator/pull/627
...
https://github.com/stackabletech/secret-operator/pull/397
...
https://github.com/stackabletech/spark-k8s-operator/pull/408

docs: provision spark dependencies
https://github.com/stackabletech/spark-k8s-operator/pull/409

Don't print Superset admin credentials during startup (#483).
https://github.com/stackabletech/superset-operator/pull/483

BREAKING: Change the username which triggers graceful shutdown from admin to graceful-shutdown-user for more expressiveness (e.g. in the Trino policies). This is a breaking change because users need to ensure that the user graceful-shutdown-user has the required permissions to initiate a graceful shutdown (#573).
https://github.com/stackabletech/trino-operator/pull/573

Grant the privileges required for graceful shutdowns to the admin user in the OPA rego rules
https://github.com/stackabletech/trino-operator/pull/575
