...
For example Druid can xref:0.6@druid::usage.adoc#_s3_for_ingestion[ingest data from S3] and also xref:0.6@druid::usage.adoc##_s3_deep_storage[use S3 as a backend for deep storage], Spark can use an xref:0.3@spark-k8s::usage.adoc#_s3_bucket_specification[S3 bucket] to store application files and data.
