...
The AuthenticationClass is the main resource required to configure the products, but it depends on some other resources. Below you will create a Secret, SecretClass and the AuthenticationClass. The following image shows the parts that are already there in blue and the parts you will add in green:

image::image$openldap-tutorial.drawio.svg[]

Use `kubectl apply` to deploy the manifests shown below.
