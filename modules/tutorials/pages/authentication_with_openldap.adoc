...
* a k8s cluster available, or {kind}[kind{external-link-icon}^] installed
...
* basic knowledge of how to create resources in Kubernetes (i.e. `kubectl apply -f <filename>.yaml`) and inspect them
  (`kubectl get` or a tool like {k9s}[k9s{external-link-icon}^])
...
Fetch the existing SupersetCluster definition from the Kubernetes API server and save it into a `superset.yaml` file:
