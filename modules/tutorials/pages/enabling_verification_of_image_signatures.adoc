...
Now that Kyverno is installed, we can create a policy that verifies that all operator images of SDP 23.7 releases are signed by Stackable's CI pipeline (Github Actions):
...
If we would change the `subject` field in the policy to something like `https://github.com/test/*`, the policy will deny the creation of operator pods because the signature is not valid anymore.