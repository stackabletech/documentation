...
Druid requires a backing storage (so called Deep-Storage) where data - partioned by date or time - is persisted as immutable "segments". Druid can use either local storage (only appropriate for stand-alone testing - i.e. all druid components run on the same machine), S3 or HDFS. In this guide we will use S3, specifically MinIO which is an S3-implementation suitable for low-footprint scenarios.

We also need to install an SQL database for Druid to store metadata. Then, after we've deployed the Druid Operator and provisioned a cluster, we can set up the ingestion job.

=== Deploy MinIO

We deploy a MinIO instance to use as our deep storage, using the MinIO helm chart:
...
Druid requires a database to store metadata, here we use the Bitnami PostgreSQL Helm chart to deploy a PostgreSQL instance:
...
As with MinIO, the credentials will also be used in the Druid cluster definition.

=== Deploy the Stackable Druid Operator

Now we can install the Stackable Druid Operator:

[source,bash]
helm install druid-operator stackable-stable/druid-operator

WARNING: make sure this is not executed in a location that contains a folder called "druid-operator" as the helm installation will not complete successfully!

=== Deploy the Druid Secret

The MinIO access credentials are deployed as a Secret, which is later referenced in the cluster definition:
...
Druid also needs a ZooKeeper instance to operate. We already set up a ZooKeeper cluster above because NiFi and Kafka also need ZooKeeper. Now we also deploy a ZNode for Druid to use, to prevent any collisions of properties in ZooKeeper:
