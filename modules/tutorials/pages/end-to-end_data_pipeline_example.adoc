...
Throughout the tutorial you will install Operators from our Stackable Helm repository using Helm. To easily install from the Stackable repository, add a link to it to your Helm repository list:
...
Kafka will be used by NiFi to publish the individual records from the S3 data to.
...
The database name, as well as user and password are all `druid`, you will need these later when configuring our cluster to use the database.
...
The Druid web interface is accessible on the Router Pod of our cluster. The Operator created a Service for the Router, from which you port-forward the Port 8888 where the web interface is served:
